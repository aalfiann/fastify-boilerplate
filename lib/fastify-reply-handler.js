...
function custom (message, code, obj) {
  const data = {
    message: message,
    statusCode: code
  }
...
async function replyHandlerPlugin (app) {
   ...
   */
  if (!app.hasReplyDecorator('success')) {
    app.decorateReply('success', function (message, obj) {
  ...
  /**
   * Reply Mongoose Error (400)
   * @param {MongooseError} mongooseError      this is the object from MongooseError
   */
  if (!app.hasReplyDecorator('mongooseError')) {
    app.decorateReply('mongooseError', function (mongooseError) {
      return this.code(400).send(mongooseError)
    })
  }
